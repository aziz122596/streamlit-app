...
def load_model():
    ...
    model.load_state_dict(torch.load('vae_model3.pth', map_location=torch.device('cpu')))
    model.eval()
    return model
    ...
    if selected_option == "Главная":
        ...
        if uploaded_file:
            try:
                ...
                # Удаление столбца 'Accession', если он есть
                if 'Accession' in data.columns:
                    data = data.drop(columns=['Accession'])
                else:
                    st.warning("Столбец 'Accession' отсутствует в загруженных данных.")
                ...
                if data.isnull().sum().sum() > 0:
                    st.warning("Обнаружены некорректные или пропущенные значения. Они будут заполнены средними значениями.")
                ...
                if X_scaled.shape[1] != model.fc1.in_features:
                    st.error(f"Размерность входных данных ({X_scaled.shape[1]}) не соответствует ожидаемой моделью ({model.fc1.in_features}).")
                    st.stop()
